    ...
    <ul>
        ...
        foreach ($userActivity as $activity) {
            ...
            if (!is_null($activity['blog_title'])) {
                echo '<a href="profile.php?id=' . $activity['user_id'] . '">User ID ' . get_username($activity['user_id']) . '</a> just made a blog post about "' . $activity['blog_title'] . '"!<br>';
            } elseif (!is_null($activity['drink_id']) && !is_null($activity['rating']) && !is_null($activity['comment'])) {
                echo '<a href="profile.php?id=' . $activity['user_id'] . '">User ID ' . get_username($activity['user_id']) . '</a> just left a ' . $activity['rating'] . ' star rating on drink ID ' . $activity['drink_id'] . ', saying "' . $activity['comment'] . '"!<br>';
